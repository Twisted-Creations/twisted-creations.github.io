...
function addLogoGlow() {
	...
	function pulseGlow() {
		...
		// More subtle intensity changes
		const intensity = Math.sin(Date.now() / 1500) * 0.15 + 0.85; // Value between 0.7 and 1.0
		const glowSize = 8 + intensity * 4; // Between 8px and 12px
		const glowOpacity = 0.5 + intensity * 0.2; // Between 0.5 and 0.7
...
function addParticleEffect() {
	...
	// Determine the number of particles based on a device - fewer for a cleaner look
...
// Create a single particle - more subtle and professional
function createParticle(container) {
	const particle = document.createElement("div");

	// More subtle random properties
	const size = Math.random() * 3 + 1; // 1-4px (smaller particles)
	const xPos = Math.random() * 100; // 0-100%
	const duration = Math.random() * 12 + 15; // 15-27s (slower movement)
	const delay = Math.random() * 8; // 0-8s (more varied start times)
	const xDrift = `${Math.random() * 60 - 30}px`; // -30px to +30px drift (less movement)

	// Set particle styles - more professional appearance
	...
	particle.style.backgroundColor = "rgba(255, 255, 255, 0.15)"; // More transparent
	particle.style.boxShadow = `0 0 ${size / 3}px rgba(42, 62, 177, 0.3)`; // Brand color glow
	particle.style.opacity = "0";
	particle.style.setProperty("--x-drift", xDrift);
	particle.style.animation = `float-up ${duration}s cubic-bezier(0.19, 1, 0.22, 1) ${delay}s infinite`;
...
function addGradientMovement() {
	...
	// Create a subtle moving gradient overlay - more professional
	...
	gradientOverlay.style.background =
		"radial-gradient(circle at 50% 50%, rgba(42, 71, 179, 0.06), transparent 75%)";
	...
	function moveGradient() {
		...
		angle += 0.1; // Slower movement
		const radius = 6; // Less movement distance
		const x = 50 + Math.sin((angle * Math.PI) / 180) * radius;
		const y = 50 + Math.cos((angle * Math.PI) / 180) * radius;

		gradientOverlay.style.background = `radial-gradient(circle at ${x}% ${y}%, rgba(42, 71, 179, 0.08), transparent 75%)`;
...
function resumeRedirect() {
	...
	if (redirectMessage && loadingBar) {
		...
		const fallbackTimer = setTimeout(() => {
			if (redirectMessage.style.opacity === "0") {
				redirectMessage.style.opacity = "1";
			}
		}, 10000); // 10 seconds, should be longer than animation
