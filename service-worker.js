...
// Additional assets that can be cached as they're requested
const ADDITIONAL_ASSETS = [
    "/Pages/about-us.html",
    "/Pages/our-team.html",
    "/Pages/game-info.html",
    "/Pages/devlog.html", // Fixed capitalization
];

// Combine all assets for the installation event
const ALL_ASSETS = [...CORE_ASSETS, ...ADDITIONAL_ASSETS];
                ...
                return Promise.all(
                    cacheNames
                        .filter((cacheName) => {
                            ...
                            return cacheName.includes('twisted-creations') &&
            ...
            caches.keys().then(cacheNames => {
                return Promise.all(
                    cacheNames
                        ...
                        .map(cacheName => {
                            console.log('Clearing cache due to preference change:', cacheName);
                            return caches.delete(cacheName);
                        })
                );
            }).then(() => {
...
self.addEventListener("fetch", (event) => {
    ...
    if (url.pathname.includes('effects-preference') ||
    ...
    return fetch(request)
        ...
        .catch(() => {
            ...
            return caches.match(request)
                .then(cachedResponse => {
                    return cachedResponse || new Response('Network error occurred. Please try again when you have a connection.', {
                        status: 408,
